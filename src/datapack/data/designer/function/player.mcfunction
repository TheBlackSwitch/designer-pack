...
# wait for the chunk to properly load
execute if score @s d.state matches -1:
    execute if loaded ~ ~ ~ run function designer:to_main
    execute if score @s d.was_forceloaded matches 0 run forceload remove ~ ~
    

# When a player somehow exits a plot, teleport them back to the survival world
execute if score @s d.state matches 1 unless dimension designer:plot_1 unless dimension designer:plot_2 unless dimension designer:plot_3 unless dimension designer:plot_3 unless dimension designer:plot_4 unless dimension designer:plot_5:
    execute store success score @s d.was_forceloaded run forceload query ~ ~
    scoreboard players set @s d.state -1
    forceload add ~ ~

execute unless score @s d.state matches -1.. run scoreboard players set @s d.state 0
